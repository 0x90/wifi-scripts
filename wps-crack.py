#!/usr/bin/env python
'''
wps-crack este software fué escrito por....
¿y qué más da?
    ...
    def gen_pin(self):
        if self.has_timeout == False and self.rcved_m3 == True:
            if self.got_fist_half == True:
                    pin_int = int(self.pin[0:8]) + 1
		    self.pin = '%08i' % (pin_int)
	    else:
		    most_sig = int(self.pin[0:4])+1
		    print '%04i' % (most_sig)
	            less_sig = int(self.pin[5:9])
                    print '%04i' % (less_sig)
                    self.pin = '%04i%04i' % (most_sig,less_sig)
